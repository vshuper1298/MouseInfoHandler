...
const int Server::portNumber = 13374;
...
void Server::receiveData(){
	...
	int newData = 0;
	std::ofstream myfile;
	myfile.open("mouse_info.txt");
	while (newData != -1){
		newData = recv(thisSocket, buffer, BUFFERSIZE, 0);
		int *mouseData = (int *)buffer;
		mouseInfo.setX(mouseData[0]);
		mouseInfo.setY(mouseData[1]);
		mouseInfo.setClick(mouseData[2]);
		myfile << mouseInfo.getX() << "x" << mouseInfo.getY() << " " << mouseInfo.getClick() << std::endl;
	}
	myfile.close();
}
